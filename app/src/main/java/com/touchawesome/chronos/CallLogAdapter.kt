 ...
 */
class CallLogAdapter(log: ArrayList<CallLogEntry>) : RecyclerView.Adapter<CallLogAdapter.CallLogVH> () {

    val data: ArrayList<CallLogEntry> = log;
    ...
    class CallLogVH (itemView: View) : RecyclerView.ViewHolder (itemView) {
        fun bind(item: CallLogEntry) {
            ...
            Picasso.with(itemView.context)
                   .load(item.photo).resize(200, 200)
                    .centerCrop()
                    .into(itemView.call_log_image)

            itemView.call_log_date.text = DateUtils.formatElapsedTime(System.currentTimeMillis() / 1000 - item.timestamp)
        }
    }
}